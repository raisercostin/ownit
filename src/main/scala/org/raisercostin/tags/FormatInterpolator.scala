...
case class FormatInterpolator(tags: Map[String, String]) {
  ...
  private def expandMultiple(selector: String, convertor: Option[String]): Try[String] = {
    ...
    all.toStream.flatMap(extractValue).headOption match {
      ...
      case Some(value) =>
        Formats.convert(value, convertors.headOption, convertors.drop(1).headOption) //.find(_.isSuccess).headOption.getOrElse(Failure(new RuntimeException("Couldn't find format")))
    }
