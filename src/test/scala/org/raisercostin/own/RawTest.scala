...
class RawTest extends FunSuite with BeforeAndAfterAll {
  ...
  test("extract exif from one file") {
    val tags = ExternalExifExtractor(false).extract(Locations.classpath("MVI_2366.MOV"))
    assertEquals(1, tags.size)
    assertEquals(69, tags(0).tags.size)
  }
  test("extract exif from one pair too") {
    val tags = ExternalExifExtractor(true).extract(Locations.classpath("MVI_2366.MOV"))
    assertEquals(1, tags.size)
    assertEquals(214, tags(0).tags.size)
  }
  ...
  def extract(file:String,extractor:Extractor):String = extractor.extract(Locations.classpath(file))(0).tags("exifCreateDate")  
  //on G11
  ...
  test("times1-sanselan") {
    println(SanselanExifExtractor.extract(Locations.classpath("time1-IMG_2384.JPG"))(0).tags.mkString("\n"))
    checkTime(new DateTime(2015, 1, 9, 0, 0, 36, local), extract("time1-IMG_2384.JPG",SanselanExifExtractor))
  }
  test("times1-fileAttribute") {
    println(FileAttributesExtractor.extract(Locations.classpath("time1-IMG_2384.JPG"))(0).tags.mkString("\n"))
